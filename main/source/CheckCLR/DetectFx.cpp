...
bool CheckNetfxVersionUsingMscoree(const TCHAR *pszNetfxVersionToCheck)
{
	...
	if (NULL != hmodMscoree)
	{
		...
		if (NULL != pfnCORBINDTORUNTIME)
		{
			...
			hr = pfnGETREQUESTEDRUNTIMEINFO(NULL, pszNetfxVersionToCheck, NULL, STARTUP_LOADER_OPTIMIZATION_MULTI_DOMAIN_HOST, dwRuntimeInfoFlags, szDirectory, CountOf(szDirectory), &dwDirectoryLength, szRetrievedVersion, CountOf(szRetrievedVersion), &dwLength);
...
******************************************************************/
bool RegistryGetValue(HKEY hk, const TCHAR * pszKey, const TCHAR * pszValue, DWORD dwType, _Out_cap_(dwSize)LPBYTE data, DWORD dwSize)
{
	HKEY hkOpened;
